...
public class ProjectileUtilities {
    public static <E extends Projectile> E shootFromTo(E projectile, LivingEntity shooter, LivingEntity target, float speed, Vec3 offset){
        return shootFromTo(projectile, shooter, target.position().add(0, target.getBbHeight() / 2, 0), speed, offset);
    }

    public static <E extends Projectile> E shootFromTo(E projectile, LivingEntity shooter, Vec3 target, float speed, Vec3 offset){
        ...
        projectile.setDeltaMovement(target.subtract(projectile.position()).normalize().multiply(speed, speed, speed));
